...
def test_transpose5():
    ...
    :return:
    """
    # Expected gotten from http://www.dcode.fr/transposition-cipher
    # original_text is just random letters from ["A", "D", "F", "G", "V", "X"] (even amount of letters)
    # random key
    after_transpose = ""
    key = (3, 4, 2, 5, 0, 6, 1)
    original_text = ""
    result = reverse_transpose(after_transpose, key)
    assert result == original_text, f"Expected {original_text} but got {result}"
...
from vigenere.vigenere import (get_indices, get_index_spacing, check_dividers, get_key, most_used_letter,
                               ...
                               find_best_key_length, solve_vigenere, find_best_shift)
...
def test_find_best_key_length():
    """
    Test the find best key length method
    """
    cipher = ("LIAKYQOXFOFJKGILSYBHYXHQSWYZPYDJMBQQRETOFCORGXYQOWGXGCWELICCKVQKRBRETOWRKVROHRYEEOXFOYLSZCBWGDCMPELDAC"
              "BTFKWBOGGNIBDSYMUSSVCKRCGSLVMLOPCKVLSREZPYDJMBQGXSPNIPDSQETNYVRDLCVSAKPCMSLYQWKRBWEIOYQOSDQSTOVLWILDWSLW"
              "GNMCCXFOYLSZCBWGDCFKWBOGGNIBDSYCORRIYXWWWSPOWCKVARKPYYNDSBOZCVSNDLCZPYDJMBQRYKCDLCBAGDLRRIDVIKSWF"
              "CXYBXZOGYEWCDLCLPYMOZYEPNELNWGCENVERPSPWWFKZCLICXMLEWCCMLMIKKRWIIYBWYXHFKZCCXYBXCNXMKKCDLCERGFIPCMR"
              "ISDKRRGIPZLYCHCMMBOHRYEAAYGBIYXIUYRJSRCVIYBRGXKNVERPSPWMLYVBOVRYWSZTMBXRRIJYGYVIAYRMWCYXHKKOCEWCYJEYZCB"
              "RKORRCYZCMBSIQDLCERGFIPCMRILYCHCMMBOHRYEQUXFOELCCKYVCCIYBGFQVMETRYHCFIJYTRRINVERPSPWXMQIRRIPGMRRXFOJJOQG"
              "CLQDEPD")
    assert find_best_key_length(cipher, 10) == 3

    cipher2 = """""".replace("\n", "").replace(" ", "").upper()
    assert find_best_key_length(cipher2, 10) == 7

    # Test for multiples of a previous key length (8 = 4 * 2)
    cipher3 = ("TPGUNWPLZPFFTGVTGLVXTROKADEJEEEXGCQMAEGWTPIHBRFKWDMHVIXSFJCYTVDSFOLUOIDLSCXYWXZSYPXBXYYANPVMBXJGXLRN"
               "PICHZLWXXGTVWOXITGBMACIUGIHGFWMHXPPSJYMHZTWSLQSLFMYGJOILMSDMHASLMXSWDZGUEINGFZQSTROESVIOLIZXYZZYKRXW"
               "FEWOUWTVAPWNAIFFAGILLMEQZLWXXGTVWOXITWVLZPEHLCXGJPWYTVNZYCSOIXZVWGIFHTEZWAPUMJZJEESAXXSWJHMNAXSWXWIG"
               "BWSKLLVNUINSMDINAIMDSNOVHECVSYHMBWLHDLXZHVXKZLZYUIPFAYYMXWTFUPQUGCJWSCWUGHSSNPWNTVEWVESUZIEZWFRCOICK"
               "AECIYEYLOPVJAEDVWNMXXHEGSNUOBVPSFPAIGPTFWWIUKRTFYAPUMJZJETRIKHPJLZWOITZJLELYESNSDPGIGSXQSYHGTOPMKPSZ"
               "ZSGWJYQYGXDMTDMXBIDLZPYHBZPJKTXSAEDVWNMXXHEGSDONAILFKJQIKIDWSCGBZVZMHESXXZPDGAXBXTWSLQSLFXZYWELYKAT"
               "LZELYYPPEADLMMECL")

    assert find_best_key_length(cipher3, 10) == 8


def test_solve_vigenere():
    """
    Test the solve vigenere method
    """
    cipher = ("TPGUNWPLZPFFTGVTGLVXTROKADEJEEEXGCQMAEGWTPIHBRFKWDMHVIXSFJCYTVDSFOLUOIDLSCXYWXZSYPXBXYYANPVMBXJGXLRN"
              "PICHZLWXXGTVWOXITGBMACIUGIHGFWMHXPPSJYMHZTWSLQSLFMYGJOILMSDMHASLMXSWDZGUEINGFZQSTROESVIOLIZXYZZYKRXW"
              "FEWOUWTVAPWNAIFFAGILLMEQZLWXXGTVWOXITWVLZPEHLCXGJPWYTVNZYCSOIXZVWGIFHTEZWAPUMJZJEESAXXSWJHMNAXSWXWIG"
              "BWSKLLVNUINSMDINAIMDSNOVHECVSYHMBWLHDLXZHVXKZLZYUIPFAYYMXWTFUPQUGCJWSCWUGHSSNPWNTVEWVESUZIEZWFRCOICK"
              "AECIYEYLOPVJAEDVWNMXXHEGSNUOBVPSFPAIGPTFWWIUKRTFYAPUMJZJETRIKHPJLZWOITZJLELYESNSDPGIGSXQSYHGTOPMKPSZ"
              "ZSGWJYQYGXDMTDMXBIDLZPYHBZPJKTXSAEDVWNMXXHEGSDONAILFKJQIKIDWSCGBZVZMHESXXZPDGAXBXTWSLQSLFXZYWELYKAT"
              "LZELYYPPEADLMMECL")
    solution = solve_vigenere(cipher=cipher, max_key_len=10)
    assert solution["key"] == "SLEUTELS"
    assert solution["text"] == ("BECAUSETHEBLACKBOARDANDSISAPLATFORMSHAVEBEENINUSESINCEMANYYEARSANDHAVESTARTEDTO"
                                "AGETHEUNIVERSITYOFANTWERPHASDECIDEDTOACQUIREANEWONLINELEARNINGPLATFORMINORDERTO"
                                "SUPPORTTHELOCALECONOMYANDMAKEUSEOFGOVERNMENTSUBSIDIESTHEUNIVERSITYHASDECIDEDTOASK"
                                "THEANSYMORESEARCHGROUPTODEVELOPTHEPLATFORMTOGETHERWITHTHEFLEMISHSTARTBECAUSETHEB"
                                "LACKBOARDANDSISAPLATFORMSHAVEBEENINUSESINCEMANYYEARSANDHAVESTARTEDTOAGETHEUNIVE"
                                "RSITYOFANTWERPHASDECIDEDTOACQUIREANEWONLINELEARNINGPLATFORMINORDERTOSUPPORTTHELOC"
                                "ALECONOMYANDMAKEUSEOFGOVERNMENTSUBSIDIESTHEUNIVERSITYHASDECIDEDTOASKTHEANSYMORESE"
                                "ARCHGROUPTODEVELOPTHEPLATFORMTOGETHERWITHTHEFLEMISHSTART")


def test_find_best_shift():
    """
    Test the find best shift method
    """
    from util.letter_frequency_table import ENGLISH
    crypt1 = "ILJHBZLAOLISHJRIVHYKHUKZPZHWSHAMVYTZOHCLILLUPUBZLZPUJLTHUFFLHYZHUKOHCLZAHYALKAV"
    assert find_best_shift(crypt1, ENGLISH) == 'H'

    crypt2 = "KNLJDBNCQNKUJLTKXJAMJWMBRBJYUJCOXAVBQJENKNNWRWDBNBRWLNVJWHHNJABJWMQJENBCJACNMCX"
    assert find_best_shift(crypt2, ENGLISH) == 'J'
